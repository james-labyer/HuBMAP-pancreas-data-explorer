...
im = iio.imread("assets/optical-clearing-czi/P1-7A/P1_7A2_KRT CD31 INS tile_Stitch.tif")
vols = []
...
txt2 = register_page(
    __name__,
    path="/p1-7a-optical-clearing/p1-7a-optical-clearing-7",
    title="P1-7A optical clearing #7",
)
...
logging.debug(
    f"Added slicer0 with {slicer0.nslices} slices to P1-7A-optical-clearing-7"
)
vol1 = vols[1]
slicer1 = VolumeSlicer(get_app(), vol1)
slicer1.graph.config["scrollZoom"] = False
logging.debug(
    f"Added slicer1 with {slicer1.nslices} slices to P1-7A-optical-clearing-7"
)
vol2 = vols[2]
slicer2 = VolumeSlicer(get_app(), vol2)
slicer2.graph.config["scrollZoom"] = False
logging.debug(
    f"Added slicer2 with {slicer2.nslices} slices to P1-7A-optical-clearing-7"
)
layout = [
    html.Section(
        id="P1-7A-optical-clearing-7",
        children=[
            html.Header(html.H2("View P1-7A optical clearing #7")),
            html.P("P1_7A2_KRT CD31 INS tile_Stitch.czi"),
            html.Div(
                [
                    dbc.Card(
                        dbc.CardBody(
            ...
            html.Div(
                children=[
                    ...
                    dbc.Button(
                        "Download",
                        id="btn-download-P1-7A-optical-clearing-7",
                        className="download-button",
                    ),
                    dcc.Download(id="download-P1-7A-optical-clearing-7"),
                ]
            ),
        ],
    ),
...
@callback(
    ...
    Input("P1-7A-optical-clearing-7", "children"),
    Input(slicer0.slider, "value"),
)
def apply_overlay0(level, children):
    logging.info("Creating overlay on P1-7A-optical-clearing-7")
...
@callback(
    Output("download-P1-7A-optical-clearing-7", "data"),
    Input("btn-download-P1-7A-optical-clearing-7", "n_clicks"),
    prevent_initial_call=True,
)
def download_file(n_clicks):
    logging.info("Sending P1_7A2_KRT CD31 INS tile_Stitch.czi")
    return dcc.send_file(
        "assets/optical-clearing-czi/P1-7A/P1_7A2_KRT CD31 INS tile_Stitch.czi"
    )
