...
logging.debug(
    f"Added slicer0 with {slicer0.nslices} slices to P1-14A-optical-clearing-4"
)
vol1 = vols[1]
slicer1 = VolumeSlicer(get_app(), vol1)
slicer1.graph.config["scrollZoom"] = False
logging.debug(
    f"Added slicer1 with {slicer1.nslices} slices to P1-14A-optical-clearing-4"
)
layout = [
    breadcrumb,
    html.Section(
        ...
        children=[
            ...
            html.P(
                "P1_14A1_KRT green INS white stack higher res_Maximum intensity projection.czi"
            ),
            html.Div(
                [
                    html.Div(
...
def apply_overlay(level, children):
    logging.info("Creating overlay on P1-14A-optical-clearing-4")
