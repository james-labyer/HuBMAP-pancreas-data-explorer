...
im = iio.imread(
'assets/optical-clearing-czi/P1-14A/P1_14A2_CD31 red INS white stack high res_Maximum intensity projection.tif'
)
vols = []
...
logging.debug(
    f'Added slicer0 with {slicer0.nslices} slices to P1-14A-optical-clearing-11'
)
vol1 = vols[1]
slicer1 = VolumeSlicer(get_app(), vol1)
slicer1.graph.config["scrollZoom"] = False
logging.debug(
    f'Added slicer1 with {slicer1.nslices} slices to P1-14A-optical-clearing-11'
)
layout = [
    html.Section(
        id="P1-14A-optical-clearing-11",
        children=[
            html.Header(html.H2("View P1-14A optical clearing #11")),
            html.P("P1_14A2_CD31 red INS white stack high res_Maximum intensity projection.czi"),
            html.Div(
                [
                    html.Div(
                        children=[
            ...
            html.Div(
                children=[
                    ...
                    dbc.Button(
                        "Download .czi",
                        id="btn-download-P1-14A-optical-clearing-11",
                        className="download-button",
                    ),
                    dcc.Download(id="download-P1-14A-optical-clearing-11"),
                ]
            ),
        ],
    ),
...
@callback(
    ...
    Input("btn-download-P1-14A-optical-clearing-11", "n_clicks"),
    prevent_initial_call=True,
)
def download_czi(n_clicks):
    logging.info(
        "Sending P1_14A2_CD31 red INS white stack high res_Maximum intensity projection.czi"
    )
    return dcc.send_file(
        "assets/optical-clearing-czi/P1-14A/P1_14A2_CD31 red INS white stack high res_Maximum intensity projection.czi"
    )
